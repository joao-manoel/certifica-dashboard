...
async function run<T>(operation: () => Promise<T>) { try { return { success: true as const, data: await operation() } } catch (error) { if (error instanceof HTTPError) { const body = await error.response.json<{ message?: string; errors?: Record<string, string[] | undefined> }>().catch(() => null); const validationMessage = body?.errors ? Object.values(body.errors).flat().find((message): message is string => Boolean(message)) : undefined; return { success: false as const, message: validationMessage || body?.message || 'Não foi possível concluir a operação.' } } return { success: false as const, message: 'Erro inesperado. Tente novamente.' } } }
